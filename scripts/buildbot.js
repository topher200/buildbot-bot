...
module.exports = function(robot) {
  ...
  robot.respond(/build (.*) on (.*)/i, function(res) {
    ...
    return robot.http(BUILDBOT_URL + "/json/builders/" + builder)
  ...
  return notifier = new cronJob(CRON_TIME_EVERY_10_SECONDS,
          function() {
              ...
              builds.forEach(function(build)) {
                ...
                robot.http(BUILDBOT_URL + "/json/builders/" + build.builder + "/builds/" + build.buildId)
                  ...
                  .get()(function(err, result, body) {
                    ...
                    if ((status.text == null) || (status.text.length === 0)) {
                      return;
                    } else if (Array.from(status.text).includes('failed')) {
                      robot.messageRoom(build.room, `Build ${branch} on <${builder}|${BUILDBOT_URL}/builders/${builder}/builds/${buildId}> failed`);
                      return build.responded = true;
                    } else if (Array.from(status.text).includes('successful')) {
