...
module.exports = (robot) ->
  ...
  robot.respond /buildbot builds/i, (res) ->
    builds = robot.brain.get 'builds'
    if not builds?
      return
    for build in builds
        ...
        robot.http(BUILDBOT_URL + "/json/builders/" + build.builder + "/builds/" + build.buildId)
          ...
          .get() (err, result, body) ->
            ...
            if not status.text? or status.text.length == 0
              res.send "#{build.builder}:#{build.buildId} still building"
            else if 'failed' in status.text
              robot.messageRoom build.user, "#{build.builder}:#{build.buildId} failed"
              build.responded = true
            else if 'successful' in status.text
              robot.messageRoom build.user, "#{build.builder}:#{build.buildId} succeeded"
              build.responded = true
            else
              robot.messageRoom build.user, "#{build.builder}:#{build.buildId} unknown status #{status.text}"
